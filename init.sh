...
echo "Fetching flake..."
curl -sSL "https://raw.githubusercontent.com/johnvilsack/init/refs/heads/main/mac/nix/flake.nix" -o "$HOME/.config/nix/flake.nix"
...
cp -f  ~/.scripts/shell/setup/mac/configs ~/.zshrc
cp -fa ~/.scripts/shell/setup/mac/nix ~/.config/
cp -fa ~/.scripts/shell/setup/mac/.setup ~/.config/
cp -fa ~/.scripts/shell/setup/mac/powershell ~/.config/